...
std::vector<cluster> RecoverIncomplete(std::vector<cluster>, std::vector<dg_cell>);
...
int Testing(std::string input) {
    ...
    TH1F* ClustPE1 = new TH1F("ClustPE1", "Distribution of the Cluster PE", 100, 0, 1000);
    TH1F* CellPE1 = new TH1F("CellPE1", "Distribution of the Cell PE", 100, 0, 300);
    TH1F* ClustPE2 = new TH1F("ClustPE2", "Distribution of the Cluster PE", 100, 0, 1000);
    TH1F* EventPE = new TH1F("EventPE", "Distribution of the Event PE", 100, 0, 1000);
    TH1F* CellPE2 = new TH1F("CellPE2", "Distribution of the Cell PE", 100, 0, 300);
    TH1F* ClustSize = new TH1F("ClustSize", "Distribution of the Clust Size", 20, 0, 20);
    TH2F* PeVsEner = new TH2F("PeVsEner", "PeVsEner;Energy[MeV];PE", 100, 0., 1000., 100, 0, 1000);
    ...
    for (int i = 0; i < TotHits; i++) {
        ...
        for (int j = 0; j < clust.size(); j++) {
            ...
            cout <<"Energia Cluster: "<< clust.at(j).e << endl;
        ...
        for (int j = 0; j < clust.size(); j++) {
            double cluspe1 = 0;
            double cluspe2 = 0;
            ClustSize->Fill(clust.at(j).cells.size());
            //if (clust.at(j).cells.size() <= 4) continue;
            for (int k = 0; k < clust.at(j).cells.size(); k++) {
                double cellpe1 = clust.at(j).cells.at(k).adc1 * 0.25;
                double cellpe2 = clust.at(j).cells.at(k).adc2 * 0.25;
                cluspe1 = cluspe1 + cellpe1;
                cluspe2 = cluspe2 + cellpe2;
                CellPE1->Fill(cellpe1);
                CellPE2->Fill(cellpe2);
            }
            cluspe = cluspe + cluspe1;
            ClustPE1->Fill(cluspe1);
            ClustPE2->Fill(cluspe2);
        }
        EventPE->Fill(cluspe);
        PeVsEner->Fill(CluEn, cluspe, 1);
        //PeVsEner->Fill(y, CluEn);
        clust.clear();
        f_clust.clear();
        og_clust.clear();
    }
...
std::vector<cluster> RecoverIncomplete(std::vector<cluster> clus, std::vector<dg_cell> incomplete_cells) {
    for (int i = 0; i < incomplete_cells.size(); i++) {
        int isbarrel = 0;
        if (incomplete_cells.at(i).mod == 30) isbarrel = 1;
        ...
        for (int j = 0; j < clus.size(); j++) {
            ...
            double clus_theta = atan((clus.at(j).z - 23910.00) / (clus.at(j).x + 2384.73)) * 180 / 3.14159265358979323846;
...
std::vector<cluster> TrackFit(std::vector<cluster> clu_vec) {
    ...
    for (int i = 0; i < clu_vec.size(); i++) {
        double apx[3], eapx[3] = { 0,0,0 }, ctrk[3] = { 0,0,0 }, ectrk[3] = { 0,0,0 };
